...
<script>
    ...
    function saveRolePower(){
        ...
        // alert(powerIds)
        $.ajax({
            url:"<%=request.getContextPath() %>/role/saveRolePower",
            type:"post",
    ...
    function initPowerTree(id){
        ...
        $("#orgTree").tree({
            url:"<%=request.getContextPath() %>/role/authTree2?id="+id ,
            checkbox:true,
            parentField:"pid",
    ...
    function deleteById(id){
        ...
        $.messager.confirm("提示","是否确定删除!",function(r){
            if(r){
                $.ajax({
                    url:"<%=request.getContextPath() %>/role/deletePowerMenuAll",
                    type:"post",
                    data:{"id":id},
                    success:function(){
    ...
    function  savePowerMenu() {
        $("#menuForm").form("submit",{
            url:"<%=request.getContextPath() %>/role/savePowerMenu",
            success:function () {
    ...
    function initTreeTable(powerId){
        //alert(powerId)
        $('#TreeTable').datagrid({
            url:"<%=request.getContextPath() %>/role/querypowerMenu?powerId="+powerId,
