...
<script>
    ...
    function openUpdateBy(id){
        //alert(id)
        $.ajax({
            url:"<%=request.getContextPath() %>/course/queryKechengById",
            type:"post",
            data:{"id":id},
    ...
    function add(){
        $("#myForm").form("submit",{
            url:"<%=request.getContextPath() %>/course/addKechen",
            success:function(){
    ...
    function deleteByid(id){
        ...
        $.messager.confirm("提示","是否确定删除!",function(r){
            if(r){
                $.ajax({
                    url:"<%=request.getContextPath() %>/course/deleteKechenAll",
                    type:"post",
                    data:{"id":id},
                    success:function(){
    ...
    //查询
    $("#myTablek").datagrid({
        url:"<%=request.getContextPath()%>/course/querykechen",
        columns:[[
            ...
            {field:'kurl',title:'封面',formatter:function(value,row,index){
                    return "<img width='50px' height='50px' src='<%=request.getContextPath() %>/"+value+"'>";
                }},
    ...
    $(function () {
        $("#eimg").uploadify({
            ...
            //后台运行上传的程序
            'uploader': '<%=request.getContextPath()%>/course/courseuploadImg',
            'width': '100',
            'height': '24',
